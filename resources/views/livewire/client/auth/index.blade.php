<div>
    ...
    @if (!$submitCodeView)
        ...
        <div>
            <form wire:submit="sendSms(Object.fromEntries(new FormData($event.target)))" method="post"
                class="d-flex flex-column align-items-center justify-content-center gap-3 mb-4">

                <input type="text" minlength="11" maxlength="13" name="mobile" placeholder=""
            ...
            <div class="row mb-4">
                <div class="col-6 d-flex justify-content-center">
                    <button class="d-flex align-content-center gap-3 py-2 px-3 btn btn-outline-secondary">
    ...
    @else
        ...
        <div class="fs-8 text-black-50">
            <p class="my-3">:لطفا کد ارسال شده به شماره موبایل {{ $userMobile }}
                در کادر زیر وارد کنید
            </p>
        </div>
        <form wire:submit="submitUser(Object.fromEntries(new FormData($event.target)))" method="post"
            class="d-flex flex-column align-items-center justify-content-center gap-3 mb-4">

            <input type="text" maxlength="4" name="code" placeholder=""
                class="w-100 py-2 rounded-3 border border-danger text-center" />

            @if ($codeErrorMasage)
                <div class=" w-100 alert alert-danger">{{ $codeErrorMasage }}</div>
            @endif
